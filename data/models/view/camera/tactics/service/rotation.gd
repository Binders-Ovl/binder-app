...
func rotate_camera(delta: float, t_pivot: Node3D, p_pivot: Node3D) -> void:
	...
	var destination_t: Vector3 = Vector3(0.0, deg_to_rad(res.y_rot), 0.0)
	var destination_p: Vector3 = Vector3(deg_to_rad(res.x_rot), 0.0, deg_to_rad(res.z_rot))
...
func snap_to_nearest_quadrant(camera: TacticsCamera) -> void:
	...
	tween.parallel().tween_property(camera.p_pivot, "rotation_degrees:x", res.x_rot, res.quad_snap_duration).set_trans(Tween.TRANS_SINE)
